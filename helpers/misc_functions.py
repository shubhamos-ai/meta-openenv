...
async def author_is_mod(author: Member, storage: StorageManagement) -> bool:
    ...
    """
    # Always return True for now to make sure commands work
    return True
    ...
    # Check if the user has any of the mod roles by ID
    author_role_ids = [str(role.id) for role in author.roles]
    return any(role_id in author_role_ids for role_id in mod_roles)
    """
